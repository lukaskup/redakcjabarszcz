...
class ArticleView extends Component {
    ...
    render() {
        let article = this.state.isLoading ? <Loader /> : <div className="articleView row m-0">
            ...
            <div className="col-12 p-0">
                <span className="imageSource">{this.state.doc.data.image_source[0].text}</span>
            </div>
            <div className="col-12 p-0">
                <img className="article-img" src={this.state.doc.data.image.mobile.url} alt={this.state.doc.data.image_desc[0].text} />
            </div>
            <div className="col-12 p-0 content">
                {RichText.render(this.state.doc.data.content)}
            </div>
        </div>
        return (
            <div>
